...
- 여러 문항에 걸쳐 정답 번호(①~⑤)가 한쪽으로 몰리지 않도록 고르게 분산하세요.
...
① 개념 A는 개념 B와 달리 시간이 지나도 변화하지 않는다.
② 개념 A는 특정 조건에서만 개념 B로 전환될 수 있다.
③ 개념 B는 개념 A보다 항상 더 안정적인 상태를 유지한다.
④ 개념 A와 개념 B는 서로 독립적으로 작동하며 영향을 주지 않는다.
⑤ 개념 B는 개념 A의 하위 개념이며, 발생 조건이 서로 동일하다.
(※ [보기]는 이 문항처럼 별도 참고자료가 필요 없으면 아예 쓰지 않습니다)
[정답 및 해설]
1. 정답 ②
지문 3문단에서 특정 압력과 온도 조건에서만 A가 B로 전환된다고 설명하므로 ②가 적절하다. ①은 지문에 제시된 시간 경과에 따른 A의 미세한 변화 내용과 모순된다. ③은 지문에 없는 내용을 임의로 추가한 것이다. ④는 지문에서 A와 B가 상호 영향을 주고받는다고 명시한 부분과 반대된다. ⑤는 A와 B가 하위 개념 관계가 아니라 별개의 현상으로 설명되어 있어 틀렸다.
[정답표]
1번 ②"""
