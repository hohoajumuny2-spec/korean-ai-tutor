...
async def chat_with_ai(
    ...
    # 또한 채팅창은 LaTeX을 그려주지 않고 글자 그대로 보여주는 단순한 창이라,
    # \frac{}{} 같은 수식 문법을 그대로 쓰면 학생이 전혀 못 알아본다 — 그래서
    # 수학 과목은 항상 채팅으로 읽기 편한 일반 텍스트 표기만 쓰게 한다.
    math_image_note = ""
    if subj_key == "math":
        math_image_note = """

[수식 표기 방법 — 반드시 지킬 것]
이 채팅창은 LaTeX이나 마크다운 수식을 그림으로 그려주지 못하고 글자 그대로 보여줍니다.
$, \\frac{}{}, \\times, \\cdot, \\sqrt{}, \\sum, \\int, \\lim, \\begin{cases}...\\end{cases} 같은
LaTeX/마크다운 수식 문법은 절대 쓰지 마세요. 대신 이렇게 일반 텍스트로 쓰세요:
- 지수: 9^(1/4), x^2 처럼 캐럿(^)과 괄호로
- 분수: 1/4, (x+1)/(x-2) 처럼 슬래시로 (필요하면 "4분의 1"처럼 한글로 풀어써도 좋음)
- 곱셈·나눗셈: ×, ÷ 기호나 그냥 * 사용 (\\times, \\cdot 금지)
- 루트: √9, √(x+1) 처럼 √ 기호 사용 (\\sqrt 금지)
- 극한·시그마·적분: "x가 0으로 갈 때", "k=1부터 4까지의 합" 처럼 말로 풀어쓰기
- 구간별로 정의된 함수(case 함수): 표나 \\begin{cases} 대신 "x가 1보다 작을 때는 3x-2, x가 1
  이상일 때는 x^2-3x+a" 처럼 문장으로 풀어쓰기"""
        if has_images:
            math_image_note += """
