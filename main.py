...
async def grade_essay(
    ...
    if model is None:
        return {"success": False, "detail": "AI 연결 오류"}
    try:
...
def update_student(req: UpdateStudentRequest):
    ...
    if req.old_name != req.new_name:
        db.collection("students").document(req.old_name).delete()
...
async def create_homework(
    ...
    db.collection("homeworks").document(title).set({
        "title": title, "desc": desc, "answer_text": answer_text, 
        "answer_file": ans_url, "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    })
    return {"success": True}
...
async def submit_homework(
    school: str = Form(...), grade: str = Form(...), 
    student_name: str = Form(...), title: str = Form(...), file: UploadFile = File(...)
):
    if db is None:
        return {"success": False}
    ...
    db.collection("reports").add({
        "submitted_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 
        "student_name": student_name, "school": school, "grade": grade, 
        "task_name": title, "type": "과제 제출", "score": "제출완료", 
        "file_url": f"/{filepath}"
    })
...
async def create_board_post(
    title: str = Form(...), desc: str = Form(""), file: Optional[UploadFile] = File(None)
):
    if db is None:
        return {"success": False}
    file_url = ""
    ...
    db.collection("board").add({
        "title": title, "desc": desc, "file_url": file_url, 
...
def delete_board_post(post_id: str):
    if db:
        db.collection("board").document(post_id).delete()
    return {"success": True}
...
def create_lecture(req: LectureRequest):
    ...
    db.collection("lectures").add({
        "title": req.title, "desc": req.desc, "video_url": req.video_url, 
        "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    })
    return {"success": True}
...
async def create_exam(
    title: str = Form(...), exam_data: str = Form(...), 
...
def submit_exam(req: ExamSubmitRequest):
    ...
    return {
        ...
        "wrong_by_diff": wrong_by_diff, "potential_ab": potential_ab, 
        "potential_abc": potential_abc, "video_url": data.get("video_url", ""), 
...
async def generate_stream(
    ...
    cnt_killer: int = Form(0), cnt_semi: int = Form(0), cnt_high: int = Form(0), 
    cnt_mid: int = Form(0), cnt_low: int = Form(0),
    ...
    if model is None:
        raise HTTPException(status_code=500, detail="AI 에러")
...
async def add_knowledge(
    title: str = Form(...), content: str = Form(""), files: Optional[List[UploadFile]] = File(None)
):
    if db is None:
        return {"success": False}
