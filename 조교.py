...
# ==========================================
    ...
    # ==========================================
    # 🪄 탭 8: AI 문제 출제기 (🔥 지문 분할, 선택지 기호 강제, 마크다운 원천 차단 적용)
    # ==========================================
    with tab8:
            ...
            if total_q_count == 0: st.warning("⚠️ 출제할 문항 수를 최소 1개 이상 설정해 주세요.")
                ...
                with st.status("⏳ **AI 국최 두뇌 가동 중... (아래 창에서 실시간 과정을 확인하세요)**", expanded=True) as status:
                    try:
                        ...
                        # 💡 핵심: 마크다운 금지, 지문당 5문제 강제, 선택지 원문자 강제 규칙 추가
                        q_prompt = f"""
                        ...
                        [🚨 출력 형태 강력 통제 규칙 - 반드시 지킬 것 🚨]
                        1. 마크다운 강조 절대 금지: 텍스트에 별표 두 개(**)는 절대 사용하지 마세요. 강조가 필요하면 반드시 작은 따옴표('')를 사용하세요. (예: '핵심어')
                        2. 지문당 문항 수 제한: 지문 1개당 문제는 무조건 '최대 5개'까지만 연결하세요. 6번째 문제부터는 반드시 ===지문===을 새로 출력하여 분리해야 합니다. 
                        3. 선택지 번호 기호 강제: 객관식 선택지는 기호 없이 글만 쓰면 절대 안 됩니다. 무조건 맨 앞에 원문자(①, ②, ③, ④, ⑤)를 붙여서 출력하세요.
                        ...
                        for chunk in q_response:
                            ...
                            # 💡 스트리밍 화면에서도 혹시 모를 ** 기호를 실시간으로 제거
                            display_text = full_generated_text.replace("**", "'")
                            stream_box.markdown(display_text + " ▌")
                            
                        # 💡 최종 데이터에서도 ** 기호를 ' 로 완벽하게 치환
                        full_generated_text = full_generated_text.replace("**", "'")
        ...
        if st.session_state.q_list:
            ...
            def generate_exam_pdf(title, q_list, layout_type):
                ...
                m_left = 2.5 * cm 
                m_right = 2.5 * cm
                m_top = 1.0 * cm   # 2페이지 이후 타이틀과 본문 간격을 맞추기 위해 상단 여백 축소
                m_bot = 2.0 * cm
