...
# 💡 AI 출력물 1차 텍스트 정제 (별표 치환 및 괄호 기호 변환)
            ...
            if total_q_count == 0: st.warning("⚠️ 출제할 문항 수를 최소 1개 이상 설정해 주세요.")
                ...
                with st.status("⏳ **AI 국최 두뇌 가동 중... (아래 창에서 실시간 과정을 확인하세요)**", expanded=True) as status:
                    try:
                        ...
                        3. 선택지 동그라미 기호 강제: 객관식 5지 선다형의 보기(선택지)는 반드시 맨 앞에 원문자(①, ②, ③, ④, ⑤)를 붙여서 출력하세요. 기호 없이 내용만 쓰면 절대 안 됩니다.
                            ...
                            if len(parts) > 0:
                                ...
                                for q_block in parts[1:]:
                                    if "===해설===" in q_block:
                                        ...
                                        # 💡 핵심 1. 최강 보정 엔진: AI가 기호를 빼먹었어도 정답이 숫자(1~5)면 강제 주입!
                                        ...
                                        if "5지" in type_match or "선다" in type_match or "객관" in type_match or ans_is_number:
                                            ...
                                            if len(non_empty_indices) >= 6: 
                                                # 마지막 5줄에 기호가 있는지 검사
                                                has_circle = any(re.match(r'^[①②③④⑤]', q_lines[i].strip()) for i in non_empty_indices[-5:])
                                                
                                                # 기호가 없으면 마지막 5줄을 강제로 낚아채서 ①~⑤ 주입!
                                                if not has_circle:
                                                    for i in range(5):
                                                        idx = non_empty_indices[len(non_empty_indices) - 5 + i]
                                                        # 쓸데없는 숫자, 기호, 괄호 등 찌꺼기 완벽 제거
                                                        clean_l = re.sub(r'^[\-\*\d\)\.]+\s*', '', q_lines[idx].strip())
                                                        q_lines[idx] = f"{['①', '②', '③', '④', '⑤'][i]} {clean_l}"
                                                    q_str = '\n'.join(q_lines)
        ...
        if st.session_state.q_list:
            ...
            for idx, item in enumerate(st.session_state.q_list):
                with st.expander(f"📌 {idx+1}번 문항 (클릭하여 수정)", expanded=False):
                    ...
                    new_a = text_area_safe = st.text_area(f"{idx+1}번 해설지 영역", item["a"], key=f"edit_a_{idx}", height=100)
            ...
            def generate_exam_pdf(title, q_list, layout_type):
                ...
                # 💡 핵심 2. 상하 여백 정밀 조정 (1페이지 상단 공간 대폭 축소, 2페이지부터는 1.0cm)
                m_left = 2.0 * cm 
                m_right = 2.0 * cm
                m_top = 1.0 * cm   
                m_bot = 1.5 * cm
                ...
                # 1페이지 상단 공간 약 4cm 축소하여 콤팩트하게 적용
                ...
                # 💡 핵심 3. 선택지 내어쓰기 여백을 12pt에서 15pt로 넓혀 가독성 극대화
                choice_style = ParagraphStyle('Choice_KR', fontName='KoreanFont', fontSize=8.5, leading=14, wordWrap='CJK', leftIndent=15, firstLineIndent=-15)
